...
def main(argv: list[str] | None = None) -> None:
    ...
    if not has_help_flag(forwarded_args):
        runs_dir = get_option_value(forwarded_args, "--runs-dir")
        if runs_dir is not None:
            require_existing_path(runs_dir, "--runs-dir", kind="dir")

    run_existing_script("stenosis-detection/run_yolo_compare_runs.py", forwarded_args)
