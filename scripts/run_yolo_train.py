...
def main(argv: list[str] | None = None) -> None:
    ...
    if not has_help_flag(forwarded_args):
        config = get_option_value(forwarded_args, "--config")
        data = get_option_value(forwarded_args, "--data")
        if config is not None:
            require_existing_path(config, "--config", kind="file")
        if data is not None:
            require_existing_path(data, "--data", kind="file")

    run_existing_script("stenosis-detection/run_yolo_train.py", forwarded_args)
